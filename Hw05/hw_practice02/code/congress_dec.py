...
def Decision_Tree_Learning(examples, attributes, parent_examples):
	if len(examples) == 0:
		# print("examples length is 0.\t", Plurality_Value(parent_examples))
		return Plurality_Value(parent_examples)
	elif has_only_same_classification(examples):
		# print("has only same classification.\t", examples[0][-1])
		# print("has only same classification.\t", examples[0][-1], "\n",attributes)
		return examples[0][-1]	# return the only classification which examples has
	elif len(attributes) == 0:
		# print("no attributes.\t", Plurality_Value(examples))
...
def Importance(a, examples):
	...
	for s in subsets:
		...
		# print(prob_labelA)
		# print(prob_labelB)
		# print(prob_labelA, prob_labelB, prob_labelA - prob_labelB)
...
def predictD(tree, test_example):
	# print(tree)
	# print(tree.subtree)
	# print(tree.column)
	# print(test_example[tree.column - 1])
	# print(tree.prob)
	if len(tree.subtree) < 2:
		if tree.prob[labels[0]] > tree.prob[labels[1]]:
			return list(tree.prob.keys())[list(tree.prob.values()).index(tree.prob[labels[0]])], tree.prob[labels[0]]
		else:
			return list(tree.prob.keys())[list(tree.prob.values()).index(tree.prob[labels[1]])], tree.prob[labels[1]]
	if isinstance(tree.subtree[test_example[tree.column - 1]], DecisionTree):
		return predictD(tree.subtree[test_example[tree.column - 1]], test_example)
	else:
		# print("\t\t\t\t\t\t\t\t\t\tLEAF NODE!!!!!")
		# print(tree.prob[tree.subtree[test_example[tree.column - 1]]])
		return tree.subtree[test_example[tree.column - 1]], tree.prob[tree.subtree[test_example[tree.column - 1]]]
	
# def predictD(tree, test_example):
# 	if isinstance(tree, DecisionTree):
# 		vote_num_int = tree.column
# 		chanceofD = tree.prob[labels[0]]
# 	else:
# 		return 1

# 	vote_answer = test_example[vote_num_int]
# 	chanceofD *= predictD(tree.subtree[vote_answer], test_example)

# 	return chanceofD

# def predictR(tree, test_example):
# 	if isinstance(tree, DecisionTree):
# 		vote_num_int = tree.column
# 		chanceofR = tree.prob[labels[1]]
# 	else:
# 		return 1
# 	vote_answer = test_example[vote_num_int]
...
# 	# print("{}'s answer: {}".format(tree, vote_answer))

# 	# print(tree.subtree[vote_answer])
# 	chanceofR *= predictR(tree.subtree[vote_answer], test_example)
# 	return chanceofR
...
def print_result(tree, test_data):
	numD = numR = 0
	# for row in test_data:
	# # 	# print(row)
	# 	chanceofD = predictD(tree, row)
	# 	print(chanceofD)
	# 	chanceofR = predictR(tree, row)
	# 	# print("{}: {} \t {}: {}".format(labels[0], chanceofD, labels[1], chanceofR))
	# 	party = labels[0] if chanceofD > chanceofR else labels[1]
	# 	chance = chanceofD if chanceofD > chanceofR else chanceofR
	# 	chanceNormalized = chance / (chanceofD + chanceofR)
	# 	print("{},{}".format(party, chanceNormalized))
	# 	if party == labels[0]:
	# 		numD+=1
	# 	elif party == labels[1]:
	# 		numR+=1
...
def main():
	...
	print_result(tree, new_test_data)
	# print_tree(tree)
	
	# print(tree)
	# print(tree.subtree)
	# print(tree.subtree['Yea'])
	# print(tree.subtree['Yea'].subtree)
